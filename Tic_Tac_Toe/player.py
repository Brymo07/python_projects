...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            # we're going to check that this is a correct value byt trying to cast
...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        other_player = 'O' if player == 'X' else 'X' # the other player..so whatever letter is N

        # first, we want to check if the previous move is a winner
        # this is base case
        if state.current_winner ==  other_player:
            # we should return position AND score because we need to keeep track of the score
            # for minmax to work
            return {'postion': None,
        ...
        for possible_move in state.available_moves():
            ...
            if player == max_player: # tring to maximize the the max_player
